...
export async function deleteWorkflow(req: Request, res: Response) {
  try {
    ...
    // Delete NodeExecutions for all executions tied to this workflow
    await prismaClient.nodeExecution.deleteMany({
      where: {
        execution: {
    ...
    // Delete Executions tied to this workflow
    await prismaClient.execution.deleteMany({
      where: { workflowId }
    });
